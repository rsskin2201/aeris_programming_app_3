...
export function MeterForm({ meter, onClose }: MeterFormProps) {
  ...
  function onSubmit(values: FormValues) {
    ...
    const dataToSave: Meter = {
        ...values,
        id: values.id!,
        createdAt: meter?.createdAt || format(new Date(), 'yyyy-MM-dd HH:mm:ss'),
    };
  ...
  return (
    <DialogContent className="sm:max-w-[600px]">
      <DialogHeader>
        <DialogTitle>{isEditMode ? 'Editar' : 'Crear'} Medidor</DialogTitle>
        <DialogDescription>
            {isEditMode ? 'Modifica los datos del tipo de medidor.' : 'Completa el formulario para dar de alta un nuevo tipo de medidor.'}
        </DialogDescription>
      </DialogHeader>
      <Form {...form}>
          ...
          <FormField control={form.control} name="marca" render={({ field }) => (
           ...
           {isEditMode && meter && (
               <div className="mt-4 grid grid-cols-2 gap-x-4 gap-y-2 rounded-md border bg-muted/50 p-4 text-xs text-muted-foreground">
                   <p><span className="font-semibold">Fecha Alta:</span> {meter.createdAt}</p>
